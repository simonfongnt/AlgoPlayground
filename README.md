...
### Commands
Command has specific functions in each modules. `help` command shows details of each of them.
```
...
`script list` to list out scripts in `scripts/`
```
>script list
script:
...
```
'script run' to run the script by name or index of `script list`
```
>script run 0
